...
TCHAR * CRegExp::regnode(TCHAR op){
  if (!bEmitCode) {
    regsize += 3;
    return regcode;
  }
        ...
        *flagp |= flags&SPSTART;
        ...
        else if (!paren && *regparse != _T('\0')) 
        {
                ...
                else
                {
						...
						LastError = "internal error: junk on end";
                        return NULL;
                }
                // NOTREACHED 
        }
...
TCHAR *CRegExp::regpiece(int *flagp)
{
        ...
        if (!(flags&HASWIDTH) && op != _T('?'))
        {
				...
				LastError = "*+ operand could be empty";
                return NULL;
        }
...
TCHAR *CRegExp::regatom(int *flagp)
{
        ...
        *flagp = WORST;         // Tentatively. 
        ...
        case _T(')'):
				...
				LastError = "internal error: \\0|) unexpected";
                return NULL;
                break;
        case _T('?'):
        case _T('+'):
        ...
        default: {
                ...
                if (len == 0)
                {
						...
						LastError = "internal error: strcspn 0";
                        return NULL;
                }
                ender = *(regparse+len);
...
size_t CRegExp::regrepeat(TCHAR *node)
{
        ...
        default:                // Oh dear.  Called inappropriately. 
				...
				LastError = "internal error: bad call of regrepeat";
